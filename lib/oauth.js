...
var store  = nStore('data/oauth.db');
...
exports.reuse = function (key, secret, reuse_oauth_token, cb) {
  store.get(reuse_oauth_token, function (err, doc) {
    if(err) {
      console.log("Error in oauth lookup of "+reuse_oauth_token);
      cb(err);
    } else {
      cb(null, function (url, method, cb) {
        var oa = getOA(key, secret, null);
        var request = oa[method.toLowerCase()](url, doc.oauth_access_token, doc.oauth_access_token_secret)
        cb(null, request);
      }, doc.results);
    }
  })
